...
import com.zhuanbaomao.mapper.CartMapper;
...
public class CartServiceImpl implements CartService {
    ...
    public Result<?> addItem(Long userId, CartItemDTO dto) {
        ...
        if (product == null || product.getStatus() != 1) {
            throw new BusinessException(2002, "商品不存在或已下架");
        }
        if (product.getStock() < dto.getQuantity()) {
            throw new BusinessException(2003, "库存不足");
        }
        ...
        List<CartVO.CartItemVO> items = cartList.stream().map(cart -> {
            ...
            if (product != null) {
                ...
                if (StrUtil.isNotBlank(product.getImages())) {
                    try {
                        List<String> images = JSONUtil.toList(product.getImages(), String.class);
                        if (!images.isEmpty()) productImage = images.get(0);
